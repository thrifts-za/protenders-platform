...
interface ProvincialTendersProps {
  provinceName: string;
  limit?: number;
}

export default function ProvincialTenders({ provinceName, limit = 6 }: ProvincialTendersProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Error fetching provincial tenders:', error);
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">Loading {provinceName} tenders...</p>
      </div>
    );
  }
      ...
      <Card className="text-center py-12">
        <CardContent className="pt-6">
          <p className="text-muted-foreground mb-4">
            No active tenders found for {provinceName} at the moment.
          </p>
          <Button asChild>
            <Link href="/search">
              Browse All Tenders
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Current Tenders in {provinceName}</h2>
          <p className="text-sm text-muted-foreground mt-1">
            Showing {tenders.results.length} of {tenders.total} active tenders
          </p>
        </div>
        <Button asChild variant="outline">
          <Link href={`/search?province=${provinceName}`}>
