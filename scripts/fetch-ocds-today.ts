...
function extractFromText(text: string): Enriched {
  ...
  // Place (only labeled, to avoid noisy PDFs)
  const placeLabels = [
...
function enrichedFromEtenders(row: EtendersRow): Enriched {
  ...
  if (row.briefingVenue) {
    b.venue = normalizeDelivery(row.briefingVenue);
    const mid = row.briefingVenue.match(/meeting\s*id[:\s-]*([A-Za-z0-9\s]+)/i);
...
async function main() {
  ...
  const pkg1 = await fetchJson<ReleasePackage>(buildUrl(1, pageSize, dateFrom, dateTo)).catch(() => ({ releases: [] } as ReleasePackage));
  if (Array.isArray(pkg1.releases)) releases.push(...pkg1.releases);
  if (maxPages >= 2 && pkg1.links?.next) {
    const pkg2 = await fetchJson<ReleasePackage>(pkg1.links.next).catch(async () => { await delay(500); try { return await fetchJson<ReleasePackage>(pkg1.links!.next!); } catch { return { releases: [] } as ReleasePackage; } });
    if (Array.isArray(pkg2.releases)) releases.push(...pkg2.releases);
  }

  const normalized = releases.map(normalize);
  ...
  if (scrape && normalized.length) {
    // eTenders site enrichment by tender number (safe, low volume)
    let count = 0;
    for (const n of normalized) {
      ...
      if (row) {
        ...
        count++;
        await delay(350); // be gentle
      }
    }
  }

  const output = { params: { dateFrom, dateTo, maxPages, pageSize }, meta: { count: normalized.length, baseUrl: BASE_URL }, tenders: normalized };
